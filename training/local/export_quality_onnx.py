"""
Export manuscript-quality SciBERT to quantized ONNX for Transformers.js.

Writes:
  training/local/quality_onnx/
  public/models/manuscript-quality/  (quantized only)
"""
import json
import shutil
...
BEST = HERE / "quality_best"
OUT = HERE / "quality_onnx"
APP_DEST = HERE.parent.parent / "public" / "models" / "manuscript-quality"

if not BEST.exists():
    raise SystemExit(f"{BEST} not found — run train_quality.py first")
...
cfg["id2label"] = {str(i): f"LABEL_{i}" for i in range(4)}
cfg["label2id"] = {f"LABEL_{i}": i for i in range(4)}
